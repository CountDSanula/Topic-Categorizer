...
if ans == "1":
        synopsis = input(" Write a movie synopsis and press <Enter>.\n")
        ml = MonkeyLearn('3b0345961006154ee73efe96d02c82b7ac010dc6')
        data = [synopsis]
        model_id = 'cl_ctyvMrhr'
        result = ml.classifiers.classify(model_id, data)
        
        counts = Counter(chain.from_iterable(i.keys() for i in result.body[0]['classifications']))
        
        if len(result.body[0]['classifications']) == 0: #If the synopysis doesn't belong in any categories the system will exit
            sys.exit("The synopsis did not match any existing topics")
        ...
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        for i in range(value):
            print("Tags:", result.body[0]['classifications'][i]['tag_name'], "\tConfidence:", result.body[0]['classifications'][i]['confidence'])  
    ...
    if option == "1":
        ...
        if len(result.body[0]['classifications']) == 0: #If the tweet doesn't belong in any categories the system will exit
            sys.exit("The tweet did not match any existing topics")
